...
custid = input("\033[92m Enter Customer id: \033[94m")

add = input("\033[92m Enter your NEA Location : \033[94m").upper()
...
with open(file) as a:
    ...
    if address1 or address2 in x:
        if address2 in x:
            address = ''.join(re.findall(address2 + ".* =.*", x)
                              ).replace(address2 + " = ", '')
        else:
            address = ''.join(re.findall(address1 + " =.*", x)
                              ).replace(address1 + " = ", '')
...
try:
    ...
    parameters = {'NEA_location': address, 'sc_no': sc,
